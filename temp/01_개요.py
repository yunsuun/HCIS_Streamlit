...
from modules.scoring import pd_to_hcis
...
def load_and_compute_distributions():
    ...
    [왜 캐싱?]
    - 고객 수가 많아도 화면 전환 시 재계산 방지
    - 운영용 대시보드 컨셉에 맞게 '고정된 기준' 유지
    """
    df, _ = load_base_df()
    df = ensure_id(df)
    ...
    return score_series, grade_series, decision_series, stats
...
score_s, grade_s, decision_s, stats = data
...
pd_s, score_s, grade_s, decision_s, stats = load_and_compute_distributions()
        ...
        with tab4:
            ...
            if not admin_mode:
                ...
                if uploaded_file:
                    ...
                    # 4. KPI 표시 (첫 고객 예시)
                    st.metric("PD", f"{pd_hat:.4%}")
                    st.metric("HCIS 점수", int(score))
